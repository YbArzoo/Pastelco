...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Handle file upload for beverage image
    $bev_image = $_FILES['bev_image']['name'];
    $target_dir = "img/Baverage/"; // Folder to store uploaded images
    $target_file = $target_dir . basename($_FILES['bev_image']['name']);

    // Move the uploaded file to the specified directory
    if (move_uploaded_file($_FILES['bev_image']['tmp_name'], $target_file)) {
        // Prepare SQL query to insert data into the beverages table
        $query = "INSERT INTO beverage (bev_name, bev_price, bev_image) VALUES (:bev_name, :bev_price, :bev_image)";
        
        $stmt = $db->prepare($query);
        $stmt->bindParam(':bev_name', $bev_name);
        $stmt->bindParam(':bev_price', $bev_price);
        $stmt->bindParam(':bev_image', $bev_image);

        // Execute the query
        if ($stmt->execute()) {
            echo "<p style='color: green;'>Beverage added successfully!</p>";
        } else {
            echo "<p style='color: red;'>Failed to add beverage. Please try again.</p>";
        }
    } else {
...
<body>
    ...
    <div class="container-fluid pt-5">
        <div class="section-title position-relative text-center mx-auto mb-5 pb-3" style="max-width: 600px;">
            <h1 class="display-4 text-uppercase">Add Beverage</h1>
        </div>
        ...
        <div class="container">
            <form action="" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label for="bev_name">Frosting Name:</label>
                    <input type="text" name="bev_name" id="bev_name" class="form-control" required>
                </div>
                <div class="form-group">
                    <label for="bev_price">Frosting Price:</label>
                    <input type="number" name="bev_price" id="bev_price" class="form-control" required>
                </div>
                <div class="form-group">
                    <label for="bev_image">Frosting Image:</label>
                    <input type="file" name="bev_image" id="bev_image" class="form-control-file" required>
                </div>
                <br>
