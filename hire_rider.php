...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Handle file upload for rider image (optional)
    $rider_image = $_FILES['rider_image']['name'];
    $target_dir = "img/"; // Folder to store uploaded images
    $target_file = $target_dir . basename($rider_image);

    // Move the uploaded file to the specified directory
    if (move_uploaded_file($_FILES['rider_image']['tmp_name'], $target_file)) {
        // Prepare SQL query to insert data into the riders table
        $query = "INSERT INTO rider (name, email, password, vehicle_number, delivery_completed, balance) 
                  VALUES (:name, :email, :password, :vehicle_number, :delivery_completed, :balance)";
        
        $stmt = $db->prepare($query);
        
        // Setting default values for delivery_completed and balance
        $delivery_completed = 0; // or whatever default value you need
        $balance = 0.00; // or whatever default value you need
        
        // Bind parameters
        $stmt->bindParam(':name', $rider_name);
        $stmt->bindParam(':email', $rider_email);
        $stmt->bindParam(':password', $rider_password);
        $stmt->bindParam(':vehicle_number', $vehicle_number);
        $stmt->bindParam(':delivery_completed', $delivery_completed);
        $stmt->bindParam(':balance', $balance);

        // Execute the query
        if ($stmt->execute()) {
            echo "<p style='color: green;'>Rider added successfully!</p>";
        } else {
            echo "<p style='color: red;'>Failed to add rider. Please try again.</p>";
        }
    } else {
...
<body>
    ...
    <div class="container-fluid pt-5">
        <div class="section-title position-relative text-center mx-auto mb-5 pb-3" style="max-width: 600px;">
            <h1 class="display-4 text-uppercase">Add your New Rider</h1>
        </div>
        ...
        <div class="container">
            <form action="" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="rider_name" id="rider_name" class="form-control" required>
                </div>
                <div class="form-group">
                    <label for="rider_email">Rider Email:</label>
                    <input type="email" name="rider_email" id="rider_email" class="form-control" required>
                </div>
                <div class="form-group">
                    <label for="rider_password">Temporary Rider Password:</label>
                    <input type="password" name="rider_password" id="rider_password" class="form-control" required>
                </div>
                <div class="form-group">
