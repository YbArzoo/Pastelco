...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Handle file upload for flavour image
    $flavour_image = $_FILES['flavour_image']['name'];
    $target_dir = "img/Flavours/"; // Folder to store uploaded images
    $target_file = $target_dir . basename($_FILES['flavour_image']['name']);

    // Move the uploaded file to the specified directory
    if (move_uploaded_file($_FILES['flavour_image']['tmp_name'], $target_file)) {
        // Prepare SQL query to insert data into the flavour table
        $query = "INSERT INTO flavour (flavour_name, flavour_price, flavour_image) VALUES (:flavour_name, :flavour_price, :flavour_image)";
        
        $stmt = $db->prepare($query);
        $stmt->bindParam(':flavour_name', $flavour_name);
        $stmt->bindParam(':flavour_price', $flavour_price);
        $stmt->bindParam(':flavour_image', $flavour_image);

        // Execute the query
        if ($stmt->execute()) {
            echo "<p style='color: green;'>Flavour added successfully!</p>";
        } else {
            echo "<p style='color: red;'>Failed to add flavour. Please try again.</p>";
        }
    } else {
...
<body>
    ...
    <div class="container-fluid pt-5">
        <div class="section-title position-relative text-center mx-auto mb-5 pb-3" style="max-width: 600px;">
            <h1 class="display-4 text-uppercase">Add Flavours</h1>
        </div>
        ...
        <div class="container">
            <form action="" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label for="flavour_price">Flavour Price:</label>
                    <input type="number" name="flavour_price" id="flavour_price" class="form-control" required>
                </div>
                <div class="form-group">
                    <label for="flavour_image">Flavour Image:</label>
                    <input type="file" name="flavour_image" id="flavour_image" class="form-control-file" required>
                </div>
                <br>
    ...
    <script>
        ...
        // Get the message div and display the message if exists
        const messageDiv = document.getElementById('message');
        if (status && message) {
            if (status === 'success') {
                messageDiv.innerHTML = `<p class="alert-success">${message}</p>`;
            } else if (status === 'error') {
                messageDiv.innerHTML = `<p class="alert-error">${message}</p>`;
            }
        }
    </script>
